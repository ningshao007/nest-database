...
export class ProductsController {
  ...
  @Get('popular/list')
  findPopularProducts(@Query('limit', new DefaultValuePipe(10), ParseIntPipe) limit: number) {
    return this.productsService.findPopularProducts(limit);
  }

  @Get('latest/list')
  findLatestProducts(@Query('limit', new DefaultValuePipe(10), ParseIntPipe) limit: number) {
  ...
  updateMultipleStock(@Body() updates: { productId: string, quantity: number }[]) {
  ...
  incrementSoldCount(
    ...
    return this.productsService.incrementSoldCount(id, body.quantity || 1);
  }
